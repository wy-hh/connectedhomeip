...
CHIP_ERROR BLEManagerImpl::HandleGAPConnect(const ChipDeviceEvent * event)
{
    ...
    {
        ChipLogProgress(DeviceLayer, "BLE connection established (ConnId: 0x%02x)", bt_conn_index((struct bt_conn *)(connEvent->BtConn)));
        mGAPConns++;
    }
    else
    {
    ...
    bt_conn_unref((struct bt_conn *)connEvent->BtConn);
...
CHIP_ERROR BLEManagerImpl::HandleGAPDisconnect(const ChipDeviceEvent * event)
{
    ...
    if (UnsetSubscribed((struct bt_conn *)(connEvent->BtConn)))
    {
        ...
        HandleConnectionError((struct bt_conn *)(connEvent->BtConn), disconReason);
    }
...
CHIP_ERROR BLEManagerImpl::HandleTXCharCCCDWrite(const ChipDeviceEvent * event)
{
    ...
    ChipLogDetail(DeviceLayer, "ConnId: 0x%02x, New CCCD value: 0x%04x", bt_conn_index((struct bt_conn *)(writeEvent->BtConn)), writeEvent->Value);

    // If the client has requested to enable indications and if it is not yet subscribed
    if (writeEvent->Value == BT_GATT_CCC_INDICATE && SetSubscribed((struct bt_conn *)(writeEvent->BtConn)))
    {
        // Alert the BLE layer that CHIPoBLE "subscribe" has been received and increment the bt_conn reference counter.
        HandleSubscribeReceived((struct bt_conn *)(writeEvent->BtConn), &CHIP_BLE_SVC_ID, &chipUUID_CHIPoBLEChar_TX);

        ChipLogProgress(DeviceLayer, "CHIPoBLE connection established (ConnId: 0x%02x, GATT MTU: %u)",
                        bt_conn_index((struct bt_conn *)(writeEvent->BtConn)), GetMTU((struct bt_conn *)(writeEvent->BtConn)));
    ...
    {
        if (UnsetSubscribed((struct bt_conn *)(writeEvent->BtConn)))
        {
            HandleUnsubscribeReceived((struct bt_conn *)(writeEvent->BtConn), &CHIP_BLE_SVC_ID, &chipUUID_CHIPoBLEChar_TX);
        }
    }

    bt_conn_unref((struct bt_conn *)(writeEvent->BtConn));
...
CHIP_ERROR BLEManagerImpl::HandleRXCharWrite(const ChipDeviceEvent * event)
{
    ...
    ChipLogDetail(DeviceLayer, "Write request received for CHIPoBLE RX characteristic (ConnId 0x%02x)",
                  bt_conn_index((struct bt_conn *)(c1WriteEvent->BtConn)));

    HandleWriteReceived((struct bt_conn *)(c1WriteEvent->BtConn), &CHIP_BLE_SVC_ID, &chipUUID_CHIPoBLEChar_RX,
                        PacketBufferHandle::Adopt(c1WriteEvent->Data));
    bt_conn_unref((struct bt_conn *)(c1WriteEvent->BtConn));
...
CHIP_ERROR BLEManagerImpl::HandleTXCharComplete(const ChipDeviceEvent * event)
{
    ...
    ChipLogDetail(DeviceLayer, "Indication for CHIPoBLE TX characteristic done (ConnId 0x%02x, result 0x%02x)",
                  bt_conn_index((struct bt_conn *)(c2IndDoneEvent->BtConn)), c2IndDoneEvent->Result);

    // Signal the BLE Layer that the outstanding indication is complete.
    HandleIndicationConfirmation((struct bt_conn *)(c2IndDoneEvent->BtConn), &CHIP_BLE_SVC_ID, &chipUUID_CHIPoBLEChar_TX);
    bt_conn_unref((struct bt_conn *)(c2IndDoneEvent->BtConn));
...
{
    ChipLogProgress(DeviceLayer, "Closing BLE GATT connection (ConnId %02x)", bt_conn_index((struct bt_conn *)( conId)));
                                    ...
                                    PacketBufferHandle pBuf)
{
    CHIP_ERROR err                   = CHIP_NO_ERROR;
    int status                       = 0;
    uint8_t index                    = bt_conn_index((struct bt_conn *)(conId));
...
{
    return mSubscribedConns[bt_conn_index((struct bt_conn *)(conn))];
}

bool BLEManagerImpl::SetSubscribed(bt_conn * conn)
{
    uint8_t index           = bt_conn_index((struct bt_conn *)(conn));
...
ssize_t BLEManagerImpl::HandleC3Read(struct bt_conn * conId, const struct bt_gatt_attr * attr, void * buf, uint16_t len,
                                     uint16_t offset)
{
    ChipLogDetail(DeviceLayer, "Read request received for CHIPoBLE C3 (ConnId 0x%02x)", bt_conn_index((struct bt_conn *)(conId)));
